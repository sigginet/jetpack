...
if ( ! class_exists( 'Contact_Info_Widget' ) ) {
	...
	<?php
		...
		function build_map( $lat, $lon ) {
			...
			wp_enqueue_script( "contact-info-map-js", plugins_url( 'contact-info-map.js', __FILE__ ), array( 'jquery', 'google-maps' ), 20121128 );
			wp_enqueue_style( "contact-info-map-css", plugins_url( 'contact-info-map.css', __FILE__ ), null, 20130123 );
